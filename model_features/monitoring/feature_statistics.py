...
class FeatureStatisticsBuilder:
    ...
    def build(self, df: DataFrame, dataset_name: str):
        ...
        #     if any(t in field.dataType.simpleString().lower() for t in numeric_types)
        # ]
        ...
        for col_name in numeric_cols:
            ...
            percentiles = df.select(
                F.percentile_approx(
                    col_name, [0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99]
                ).alias("percentiles")
        ...
        for col_name in categorical_cols:
            ...
            result["categorical_features"][col_name] = {
                ...
                "top_values": {
                    str(row[col_name]): int(row["count"]) for row in top_values
                },
            }
        ...
        for col_name in embedding_cols:
            stats = (
                df.select(F.size(col_name).alias("embedding_dim"))
                .agg(
                    F.count("*").alias("count"),
                    F.avg("embedding_dim").alias("avg_dimension"),
                )
                .collect()[0]
            )
            ...
            result["embedding_features"][col_name] = {
                ...
                "avg_dimension": float(stats["avg_dimension"])
                if stats["avg_dimension"] is not None
                else None,
            }

        if "embedding_available" in df.columns:
            embedding_stats = df.agg(
                F.mean("embedding_available").alias("coverage_ratio")
            ).collect()[0]

            result["embedding_statistics"] = {
                "coverage_ratio": embedding_stats["coverage_ratio"]
            }
