...
requirements_PATH="$S3_BUCKET/processed/scripts/requirements.txt"
...
# ─── STEP 3: Download requirements.txt from S3 ──────────────
echo "[$(date)] Downloading requirements from S3..."
aws s3 cp "$requirements_PATH" /tmp/requirements.txt
...
sudo "$VENV_PATH/bin/pip" install -r /tmp/requirements.txt \
    --no-cache-dir \
    --quiet
